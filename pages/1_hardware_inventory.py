...
def append_purchases(new_rows_df):
    ...
    for fy, group_df in df_save.groupby('Fiscal Year'):
        ...
        rows = [{k: _firestore_safe(v) for k, v in row.to_dict().items()} for _, row in group_df.iterrows()]
        _commit_in_batches(
            db,
...
def _firestore_safe(value):
    """Converts pandas/numpy scalar types into plain Python types the
    Firestore client can actually serialize. Values pulled out of a
    st.data_editor-edited row (edited_row[k]) come back as numpy.int64,
    numpy.float64, numpy.bool_, pandas.Timestamp, etc, NOT native Python
    types - Firestore's encode_value() only recognizes native types, so
    writing one of these raw raises "Cannot convert to a Firestore Value"."""
    if value is None:
        return None
...
def update_purchase_entry(fy_doc_id, doc_id, updated_fields):
    ...
    updated_fields = {k: _firestore_safe(v) for k, v in updated_fields.items()}
...
def _find_col(df, *candidates):
    """Case/whitespace-insensitive column lookup by name. Needed because
    mapping_df is built directly from Firestore's code_mapping documents
    (`pd.DataFrame([d.to_dict() for d in docs])`), and Firestore stores
    document fields as an unordered map - the field order you get back
    from to_dict() is NOT guaranteed to match the order they were
    written in. Relying on column POSITION (mapping_df.iloc[:, 0], etc)
    to mean "Item Code" is unsafe for exactly this reason: it can quietly
    point at "Item Name" or "Units" instead, and every code lookup built
    from it comes out wrong with no visible error. Looking columns up by
    name sidesteps the ordering issue entirely."""
...
# Looked up BY NAME, not by position - see _find_col() docstring for why
# position (iloc) is unsafe here.
code_dict = {}
unit_dict = {}
if not mapping_df.empty:
    code_col = _find_col(mapping_df, "Item Code")
    name_col = _find_col(mapping_df, "Item Name")
    unit_col = _find_col(mapping_df, "Units", "Unit")

    if code_col and name_col:
        code_dict = dict(zip(mapping_df[code_col].apply(normalize_code), mapping_df[name_col].astype(str).str.strip()))
    else:
        st.warning(
            "⚠️ code_mapping documents don't have recognizable 'Item Code' / 'Item Name' "
            f"fields (found columns: {list(mapping_df.columns)}). Item codes in bulk "
            "uploads won't be replaced with product names until this is fixed."
        )

    if code_col and unit_col:
        unit_dict = dict(zip(mapping_df[code_col].apply(normalize_code), mapping_df[unit_col].astype(str).str.strip()))
